...
PART_SDCARD="/dev/block/platform/msm_sdcc.1/by-name/userdata"
DEST_SDCARD="/data/media"

# use /data (UDA) as dalvik cache in bigdata mode
PART_DALVIK="/dev/block/platform/sdhci-tegra.3/by-name/cache"
...
function mount_vfat_sdcard() {
	...
	mount -t ext4 -o uid=1023,gid=1023,umask=0007 ${PART_SDCARD} ${DEST_SDCARD}
}
...
touch /dev/.dlx_sdcard_mount_done
